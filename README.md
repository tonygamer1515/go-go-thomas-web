...
- Original `TrackQualityController.m_SkyBoxMaterial` assets are restored per level: StarryNight for Emily’s canyon, DawnDusk for James’s docks, and Sunny2 for Percy, Thomas, and Toby. Each uses the original front/back/left/right textures; mobile receives local WebP derivatives.
